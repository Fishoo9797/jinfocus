...
| **往期** | 时间轴回溯，可切换「全家」/「我的」 |
| **我的** | 设置昵称；创建家庭 / 输入家庭ID加入；复制家庭ID分享给家人 |

- 数据存于**微信云开发**（云数据库 + 云存储 + 云函数），无需自建服务器、无需域名备案。
- 多人同属一个 `familyId` 即可互相看到日记，家人之间天然隔离。
...
| 集合 | 字段 |
|------|------|
| `users` | `openid`, `nickName`, `familyId`, `familyName`, `createdAt` |
...
- 云函数以**管理员身份**读写数据库（绕过端侧权限限制），家庭隔离逻辑写在云函数内（`familyId` 过滤），安全可靠。
